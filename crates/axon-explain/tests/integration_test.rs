...
use axon_explain::types::{
    ActionAttribution, ActionSnapshot, ContributionDirection, Explanation, FeatureContribution,
};
use chrono::Utc;
...
fn test_end_to_end_trading_decision_explanation() {
    // 1. 准备背景数据（过去 100 个观察）
    let background: Vec<Vec<f64>> = (0..100)
        .map(|i| {
            vec![
    ...
    let rsi = attr
        .feature_contributions
        .iter()
        .find(|c| c.feature_name == "rsi_14")
        .unwrap();
    assert!(
        matches!(rsi.direction, ContributionDirection::Positive),
        "RSI=65 应正向"
    );
    let bb = attr
        .feature_contributions
        .iter()
        .find(|c| c.feature_name == "bb_pct")
        .unwrap();
    assert!(
        matches!(bb.direction, ContributionDirection::Positive),
        "BB%=0.8 应正向"
    );
}
...
fn test_single_explanation_completes_under_100ms() {
    let background: Vec<Vec<f64>> = (0..50)
        .map(|i| {
            vec![
    ...
    let explainer = TradingExplainer::new(TradingModel::new(), background);
    let mut obs = HashMap::new();
    for (i, n) in ["price", "rsi_14", "volume", "macd", "atr", "bb_pct"]
        .iter()
        .enumerate()
    {
...
fn test_multi_explanation_aggregation() {
    let background: Vec<Vec<f64>> = (0..30)
        .map(|i| {
            vec![
                50_000.0 + (i as f64 * 10.0),
                50.0,
