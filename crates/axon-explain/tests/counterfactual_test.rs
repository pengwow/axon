...
use axon_explain::types::{ActionSnapshot, CounterfactualExplanation, Explanation, FeatureContribution, ActionAttribution, ContributionDirection};
...
impl Explainer for TestExplainer {
    fn explain(
        ...
        Ok(Explanation {
            ...
            action_attributions: vec![ActionAttribution::from_contributions(
                "position_size".into(), pred, 0.0, contribs,
            )],
    ...
    fn generate_counterfactuals(
        ...
        _action: &ActionSnapshot,
        _max: usize,
    ) -> Vec<CounterfactualExplanation> { vec![] }
}
...
fn test_generator_returns_counterfactuals() {
    ...
    let generator = CounterfactualGenerator::with_feature_names(
        ...
        CounterfactualConfig::new().with_max_changes(3).with_confidence_threshold(0.01),
    );
...
fn test_counterfactual_respects_max_changes() {
    ...
    let generator = CounterfactualGenerator::with_feature_names(
        Box::new(model),
        vec!["f1".into(), "f2".into(), "f3".into(), "f4".into(), "f5".into()],
        CounterfactualConfig::new().with_max_changes(2).with_confidence_threshold(0.0),
    );
    let explainer = TestExplainer {
        model: LinearModel::new(vec![0.1; 5], 0.0),
        feature_names: vec!["f1".into(), "f2".into(), "f3".into(), "f4".into(), "f5".into()],
    };
    let mut obs = HashMap::new();
...
fn test_counterfactual_narrative_includes_values() {
    ...
    // 包含原始值和新值的数字
    assert!(narrative.contains("100.00"), "叙事应包含原始值 100.00: {}", narrative);
    assert!(narrative.contains("50.00"), "叙事应包含新值 50.00（步长 1.0 均值拉近 50%）: {}", narrative);
}
