...
impl ReportGenerator {
    ...
    pub fn render_html(report: &DecisionReport) -> String {
        ...
        for (name, avg_shap) in &report.feature_summary.top_features {
            let cls = if *avg_shap > 0.0 { "positive" } else { "negative" };
        ...
        for (i, exp) in report.explanations.iter().take(20).enumerate() {
            ...
            parts.push(format!("<p><strong>摘要:</strong> {}</p>", html_escape(&exp.summary)));
            parts.push(format!("<p><strong>置信度:</strong> {:.2}%</p>", exp.confidence * 100.0));
            parts.push(format!("<p><strong>动作:</strong> position={:.4} entry={:.2} SL={:.2} TP={:.2} ({})</p>",
                exp.action.position_size, exp.action.entry_price, exp.action.stop_loss, exp.action.take_profit, exp.action.order_type));

            // Top features
            if let Some(attr) = exp.action_attributions.first() {
                parts.push("<ul>".to_string());
                for feat in attr.top_positive.iter().take(3).chain(attr.top_negative.iter().take(3)) {
                    let cls = if matches!(feat.direction, ContributionDirection::Positive) { "positive" } else { "negative" };
                    parts.push(format!(
                        "<li class=\"{}\">{}: {:+.6} (当前值: {:.4})</li>",
                        cls, html_escape(&feat.feature_name), feat.shap_value, feat.feature_value
                    ));
                }
                parts.push("</ul>".to_string());
            }
            ...
            for cf in &exp.counterfactuals {
                parts.push(format!("<p><em>反事实:</em> {}</p>", html_escape(&cf.narrative)));
            }
    ...
    pub fn render_markdown(report: &DecisionReport) -> String {
        ...
        for (i, exp) in report.explanations.iter().take(20).enumerate() {
            ...
            lines.push(format!("- **动作:** position={:.4} entry={:.2} SL={:.2} TP={:.2} ({})",
                exp.action.position_size, exp.action.entry_price, exp.action.stop_loss, exp.action.take_profit, exp.action.order_type));

            if let Some(attr) = exp.action_attributions.first() {
                for feat in attr.top_positive.iter().take(3) {
                    lines.push(format!("  - ✅ {}: {:+.6} (当前值: {:.4})",
